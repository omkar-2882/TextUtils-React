    ...
    return (
        <nav className={`navbar navbar-expand-lg navbar-${props.mode} bg-${props.mode}`}>
            <div className="container-fluid">
                <a className={`text-${props.mode !== 'light' ? 'light' : 'dark'} navbar-brand`} href="#">{props.title}</a>
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                        ...
                        {/* <li className="nav-item">
                            <Link className={`text-${props.mode !== 'light' ? 'light' : 'dark'} nav-link active" aria-current="page`} to="/">Home</Link>
                        </li> */}
                        {/* <li className="nav-item">
                            <Link className={`text-${props.mode!=='light'?'light':'dark'} nav-link`} to="/about">{props.aboutText}</Link>
                        </li> */}
                    </ul>
                    <form className="d-flex" role="search">
                        {/* <input className="form-control me-2" type="search" placeholder="Search" aria-label="Search" />
                        <button type="submit" className={`me-0 btn btn-${props.mode === 'light' ? 'dark' : 'light'}`}>Search</button> */}
                        <div className="my-0 form-check form-switch bg-light">

                            <BrightnessLow onClick={props.toggleMode} className="me-4 mt-1 ms-0 px-0" color="grey" size={33} />
